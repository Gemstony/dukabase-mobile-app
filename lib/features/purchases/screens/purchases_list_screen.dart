  ...
  Future<void> _fetchPurchases() async {
    ...
    try {
      ...
      if (snapshot.docs.isNotEmpty) {
        ...
        final newPurchases = snapshot.docs.map((doc) => PurchaseModel.fromMap(doc.id, doc.data())).toList();
        _purchases.addAll(newPurchases);
        
        // Accumulate total for loaded purchases
        _totalPurchasesLoaded += newPurchases.fold(0.0, (sum, p) => sum + p.totalAmount);
      }
    } catch (e) {
                          ...
                          return Card(
                            ...
                            child: ListTile(
                              ...
                              leading: CircleAvatar(
                                backgroundColor: purchase.status == 'completed' ? Colors.green.withOpacity(0.2) : Colors.orange.withOpacity(0.2),
                                child: Icon(
                                  purchase.status == 'completed' ? Icons.check_circle : Icons.pending,
                              ...
                              trailing: Text(
                                ...
                                style: TextStyle(
                                  color: purchase.paidAmount >= purchase.totalAmount ? Colors.green : Colors.red,
                                  fontWeight: FontWeight.bold,
                                ),
                              ),
                            ),
